...
export default function AdminPanel() {
  ...
  async function loadData() {
    ...
    const [statsRes, reportsRes] = await Promise.all([
      ...
      authFetch('/api/reports?limit=200'),
    ])
  ...
  const ROW = 'flex items-start gap-3 py-3 border-b border-[#1e2730]'
  const BADGE = 'text-[10px] font-bold px-2 py-0.5 rounded uppercase'
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px 24px' }}>
      ...
      {msg && (
        <div style={{ background: '#0d2b1a', border: '1px solid #1a4731', borderRadius: 6, padding: '10px 14px', marginBottom: 16, fontSize: 13, color: '#4ade80' }}>
          {msg}
        </div>
      )}
        ...
        {loading ? (
          ...
          <div>
            {reports.map(r => {
              ...
              return (
                <div key={r.id} className={ROW} style={{ padding: '10px 16px' }}>
